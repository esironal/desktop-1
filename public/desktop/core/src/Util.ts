...
export module array { 
    export function remove(list: Array,item: any) { 
        var i = 0,
            l = list.length;
        for (; i < l ; i++) { 
            if (list[i] == item) { 
...
export function getIconClsByType(type: string) {
    if (type === "folder") {
            return "folder"
        }
    if (type === "txt") {
...
export function setCmp(id, cmp) {
    window["cmpMap"] = window["cmpMap"] || [];
    window["cmpMap"][id] = cmp;
}
    

export module OpenMode { 

    export function setOpenMode(cfg) { 
    ...
    export function openCompoment(cfg) { 

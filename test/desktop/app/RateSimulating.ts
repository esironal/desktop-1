...
declare var Design, Simulating, addMonths2, getMonthDays, treeList;
...
setTimeout(function () {
    ...
    function showChart(store) {
        ...
        if (!store.models.length)
            return;
        var values = [],
            baseDate = new Date(store.models[store.count() - 1].get('takeEffectDate').replace(/-/g, '/')),
            before = store.models[store.count() - 1].get('value');

        for (var i = 0; i < 60; i++) {
            var date = addMonths2(baseDate, i, 2),
                month = date.getMonth(),
    ...
    var columns = <IExample5[]>[
        ...
        {
            text: '编辑', width: 200, align: 'center', editor: {
                xtype: 'container',
                display: true,
    ...
    var form = Design.widget('form', {
        ...
        items: [{
            ...
            name: 'value',
            customValidator: function (value):any {
